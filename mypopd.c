...
void handle_client(int fd) {
    ...
    char data[512];
    //define checks, auth is for password
    int auth=0;
    //user is username check
    int user=0;
    //len is length, used for pointer arithimetic
    int len=0;
    ...
    while((len=nb_read_line(netbuffer, data))){
        ...
        if(strncasecmp(p, "user",4)==0){
            //string concat to fit the methods
            char* pch = strtok (p," ");
            pch = strtok (NULL, " ");
            ...
            x=x-1;
            if(get_mail_item(maillist, x)==NULL){
                char msg[100] = "+ERROR! DELETED MESSAGE\n";
                write(fd, msg, strlen(msg));
            }else{
                //print the size
           ...
           if(user==1&&auth==1){
            ...
            char msg[100] = "+OK Deleted messages and quitting \n";
            send_string(fd, msg);
